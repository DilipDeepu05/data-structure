...
public class CloneADirectedGraph {
	...
	private static Node cloneGraph(Node org, Node newS, boolean[] visited) {
		Node clone  =null;
		if(!visited[org.key] && !org.adj.isEmpty()) {
			for(Node old : org.adj) {
	...
	private static void printGraph(Node n, boolean[] visited) {
		if(!visited[n.key]) {
			for(Node i: n.adj) {
